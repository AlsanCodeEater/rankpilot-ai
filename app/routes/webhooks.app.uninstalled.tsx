...
async function cleanupShopAfterUninstall(shop: string) {
  try {
    ...
    console.error("app/uninstalled background cleanup failed", { shop, error });
  }
}
...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    console.error("Invalid app/uninstalled webhook HMAC", error);
    return new Response("Unauthorized", { status: 401 });
  }

  setTimeout(() => {
    cleanupShopAfterUninstall(shop).catch((error) => {
      console.error("app/uninstalled async cleanup crashed", { shop, error });
    });
  }, 0);
